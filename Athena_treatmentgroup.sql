...
included_events AS (
  ...
  FROM (
    ...
    WHERE (MG.inclusion_rule_mask = cast(pow(2, 2) as bigint) - 1)
  ) Results
...
-- NEW: earliest qualifying amox/clav start per person, reusing the same
-- join logic already used inside Inclusion_1 to check the 14-day window
drug_start_dates AS (
  SELECT P.person_id, MIN(A.start_date) AS drug_start_date
  FROM qualified_events P
  JOIN (
...
-- NEW: re-anchor to drug start date instead of diagnosis date
cohort_results_anchored AS (
  SELECT cr.subject_id, 
         COALESCE(ds.drug_start_date, cr.cohort_start_date) AS anchor_date
  FROM cohort_results cr
  LEFT JOIN drug_start_dates ds ON cr.subject_id = ds.person_id
),
...
failure_flags AS (
    SELECT 
        ...
        MAX(CASE 
            ...
            THEN 1 ELSE 0 
        END) AS is_failure
    FROM cohort_results_anchored c          -- CHANGED from cohort_results
    ...
    LEFT JOIN deid_cdw_ucsf.visitfact vf 
        ...
        AND date_diff('day', c.anchor_date, vf.encounterdatekeyvalue) > 0   -- CHANGED
        AND date_diff('day', c.anchor_date, vf.encounterdatekeyvalue) <= 30 -- CHANGED
