...
**Conclusion:** while clinical notes contain rich, nuanced clinical detail, extracting drug exposure via zero-shot LLM inference introduces meaningful variance across models — some over-predict (OpenBioLLM, Mistral) and some under-predict (Qwen), with no single model achieving both high sensitivity and high specificity simultaneously. Structured medication exposure data (as used in Milestone 1) has neither failure mode by construction — a drug_exposure record either exists or it doesn't — making it the more robust mechanism for defining strict cohort inclusion criteria, even though it lacks the contextual richness (dosing rationale, treatment response, care setting) that notes can provide.
